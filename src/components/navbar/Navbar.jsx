...
const Navbar = () => {
  ...
  const activeStateConditional = active || pathname !== "/";
  ...
  return (
    ...
    >
          ...
          <div className="logo">
            <Link to="/">
              <svg width="89" height="27" viewBox="0 0 89 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g fill={active || pathname !== "/" ? "#404145" : "#ffffff"}>
            ...
            <AnimatePresence>
              {pathname !== "/" && <Search setFocusInput={setFocusInput} />}

              {showMenu && pathname === "/" && <Search setFocusInput={setFocusInput} />}
            </AnimatePresence>
          </div>
          <div
            className={classNames({
      ...
      {pathname !== "/" && <CategoryMenu />}

      {showMenu && pathname === "/" && <CategoryMenu />}
    </div>
  );
};
