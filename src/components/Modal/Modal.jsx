...
const RegisterModal = ({ type, activeModal, setAcitveModal }) => {
  return (
    <div className="modals">
      <div className="container">
        <div className="closeModal" onClick={() => setAcitveModal("none")}>
          X
        </div>
        <div className="modal-header">
          <h4>{type === "register" ? "Join Fiverr" : "Sign In to Fiverr"}</h4>
        ...
        <div className="modal-content">
          ...
          <br />
          {type === "register" ? <span>By joining I agree to receive emails from Fiverr.</span> : <span className="forgot-password">Forgot Password?</span>}
        </div>
        <div className="modal-footer">
          <p>{type === "register" ? "Already a member?" : "Not a member yet?"}</p>
          <button onClick={activeModal === "register" ? () => setAcitveModal("login") : () => setAcitveModal("register")}>{type === "register" ? "Sign In" : "Join now"}</button>
        </div>
      </div>
    </div>
  );
